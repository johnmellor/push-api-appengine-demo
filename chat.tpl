    ...
    (function() {
      ...
      function setStatus(buttonName, className, text, responseText) {
        ...
        if (className == 'success') {
          // Don't bother notifying success.
          result.textContent = "";
        }
        else {
        ...
        if (responseText) {
          var mimeType = /^\s*</.test(responseText) ? 'text/html' : 'text/plain';
          resultLink.innerHTML =
            " <a href='data:" + mimeType + ","
      ...
      window.addEventListener('DOMContentLoaded', function() {
        isFreshRegistration.then(function(freshReg) {
          if (freshReg) {
            // Clear out local data, this is a new registration,
            // so will require a new login
          ...
          if (username != null) {
            ...
            usernameEl.value = AUTO_SUBSCRIBE_USERNAME;
            joinChat();
          } else {
            usernameEl.focus();
          }
          $('#active-username').textContent = usernameEl.value;
      ...
      function joinChat() {
        ...
        swRegisterPromise.then(function(sw) {
          requestPermission();
        }, function(error) {
          console.error(error);
      ...
      function requestPermission() {
        ...
        Notification.requestPermission(function(permission) {
          ...
          if (permission == "denied") {
            setStatus(
              'join', 'fail',
              "Notification permission denied. Reset it via Page Info."
            );
          } else { // "default"
            ...
            setStatus(
              'join', 'fail',
              "Notification permission prompt dismissed. Reload to try again."
            );
          }
        });
      }
      ...
      function doSubscribe(pushManager) {
        pushManager.subscribe({userVisibleOnly: true}).then(function(ps) {
          ...
          if ('subscriptionId' in ps && !endpoint.includes(ps.subscriptionId)) {
        ...
        xhr.onload = function() {
          if (('' + xhr.status)[0] != '2') {
            ...
            if (/Firefox\/4\d\.[\d.]+$/.test(navigator.userAgent)) {
              ...
              localforage.setItem('username', $('#username').value).then(function() {
                location.reload();
              });
              return;
            }
      ...
      function updateText() {
        return localforage.getItem('messages').then(function(text) {
      ...
      $('#send-form').addEventListener('submit', function(evt) {
        ...
        xhr.onload = function() {
          ...
          if (('' + xhr.status)[0] != '2') {
            setStatus(
              'send', 'fail',
              "Server error " + statusString,
              xhr.responseText
            );
          } else {
            setStatus(
              'send', 'success',
              "Sent, status " + statusString,
              xhr.responseText
            );
            $('#message').value = "";
          }
